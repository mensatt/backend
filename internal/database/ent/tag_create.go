...
func (tc *TagCreate) defaults() {
	if _, ok := tc.mutation.Priority(); !ok {
		v := tag.DefaultPriority
		tc.mutation.SetPriority(v)
	}
	if _, ok := tc.mutation.IsAllergy(); !ok {
		v := tag.DefaultIsAllergy
		tc.mutation.SetIsAllergy(v)
	}
}
...
func (tc *TagCreate) check() error {
	...
	if v, ok := tc.mutation.ShortName(); ok {
	...
	if _, ok := tc.mutation.Priority(); !ok {
		return &ValidationError{Name: "priority", err: errors.New(`ent: missing required field "Tag.priority"`)}
	}
	...
	if _, ok := tc.mutation.IsAllergy(); !ok {
		return &ValidationError{Name: "is_allergy", err: errors.New(`ent: missing required field "Tag.is_allergy"`)}
	}
	return nil
}
...
func (tc *TagCreate) createSpec() (*Tag, *sqlgraph.CreateSpec) {
	...
	if value, ok := tc.mutation.ShortName(); ok {
...
func (u *TagUpsert) UpdateIsAllergy() *TagUpsert {
	u.SetExcluded(tag.FieldIsAllergy)
	return u
}
