...
func (iq *ImageQuery) QueryReview() *ReviewQuery {
	query := (&ReviewClient{config: iq.config}).Query()
...
func (iq *ImageQuery) Clone() *ImageQuery {
	...
	return &ImageQuery{
		...
		withReview: iq.withReview.Clone(),
...
func (iq *ImageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Image, error) {
	...
	if query := iq.withReview; query != nil {
		if err := iq.loadReview(ctx, query, nodes, nil,
			func(n *Image, e *Review) { n.Edges.Review = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (iq *ImageQuery) loadReview(ctx context.Context, query *ReviewQuery, nodes []*Image, init func(*Image), assign func(*Image, *Review)) error {
