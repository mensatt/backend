...
func (lq *LocationQuery) QueryOccurrences() *OccurrenceQuery {
	query := (&OccurrenceClient{config: lq.config}).Query()
...
//		GroupBy(location.FieldExternalID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Location.Query().
//		Select(location.FieldExternalID).
//		Scan(ctx, &v)
...
func (lq *LocationQuery) loadOccurrences(ctx context.Context, query *OccurrenceQuery, nodes []*Location, init func(*Location), assign func(*Location, *Occurrence)) error {
	...
	query.Where(predicate.Occurrence(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(location.OccurrencesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "location" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
